...
import { LOCAL_STORAGE_ACCESS_TOKEN, LOCAL_STORAGE_REFRESH_TOKEN } from "../features/auth/constants";
import { localStorageService } from "./localStorageService";

let isRefreshing = false;
let requestQueue: ((newToken: string) => Promise<void>)[] = [];

export const authorizedFetchService = async (url: string, options: RequestInit & { headers: { Authorization: string } }): Promise<Response> => {
  options.headers = {
    ...options.headers,
  ...
  if (response.status === 401) {
    if (!isRefreshing) {
      ...
      try {
        ...
        localStorageService.setItem(LOCAL_STORAGE_ACCESS_TOKEN, data.accessToken);
        isRefreshing = false;
